...
def transform(df: pd.DataFrame, df_exchange_codes: pd.DataFrame) -> pd.DataFrame:
    ...
    df_quotes_renamed = df.rename(
        columns={
            "i": "id", # id is really only the index within the api response and should not be treated as a persistent unique identifier for trades across requests
            "t": "timestamp",
            "x": "exchange",
            "p": "price",
            "s": "size",
    ...
    # for padding timestamp so it is in nanosecond format (api truncates trailing 0s); we will still be able to lexicographically sort the string version of this timestamp
    ...
    # reorder dataframe by timestamp so that the primary key autogenerated during our load to postgres is chronological (needs to happen after merge)
    df_exchange = df_exchange.sort_values("timestamp")
